...
## 5. The vision call (`house_facade.from_photo(image_b64, mime) -> (draft|None, notes, error|None)`)
...
- `PORCH_W4`, formerly listed here as write-only dead code, was deleted in the final fix wave (below); `PORCH_FRONT_Z4` now defaults to the wall face rather than 4.6 units into the grass when no main-face porch is built.
- The final whole-branch review (2026-09-15) found §4.8's "the same slot never appears twice per layer" reads as flatly contradicting §4.5's "a porch is an overlay [that] shares slots with doors and windows" — both are true, but only once "per layer" is read as "per layer, among EXCLUSIVE kinds" (a porch is never exclusive). §4.8 above now says so explicitly. The same review found and fixed the hand editor bug this ambiguity had produced (the span-merge item above): `facadeEntry()` was returning whichever entry sorted first at a slot, porch included, so a porch sorting before the window/door it overlaid blanked that cell, misrouted its selection, and made editing it delete both entries.
