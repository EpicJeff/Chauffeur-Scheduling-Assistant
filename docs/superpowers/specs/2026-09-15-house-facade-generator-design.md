...
The street elevation is one strip of slots, west to east. Slot width `SLOT_W = 1.85` (the current window rhythm: windows at -4.6/-1.9 are 2.7 apart, casing-to-casing gap 1.1; 1.85 is the pitch that lands the canonical three windows and the door on whole slots — see 2.2). Slots are derived from the four street faces in `FULL_HOUSE` and the shell constants, in this order:
...
7. **Budget caps** (constants, tuned in 7.2): `MAX_WINDOWS` (windows + dormer windows, initial 10), `MAX_DORMERS` (initial 6), `MAX_GABLES` (initial 4), at most one porch. Excess entries drop east-most first; a note names what was dropped.
...
`worst_case()` returns the heaviest spec the caps allow (every free slot a `tall` window, `MAX_DORMERS` dormers with windows, `MAX_GABLES` gables, a full-face `sitting` porch, a 3-span `glass` garage door, `clapboard`) — the budget probe's input.
