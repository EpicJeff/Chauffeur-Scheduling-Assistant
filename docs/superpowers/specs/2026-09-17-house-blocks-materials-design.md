...
- *rev* **Where blocks meet.** The two street faces may differ in depth, height and roof form. Rules: the deeper block's side wall is a real wall from the shallower face to the deeper face, in the deeper block's cladding; roofs meet by the existing valley clip against the OTHER block's deck (the buried region is the union of both decks, whichever is higher wins); a one-story block against a two-story block gets its roof clipped by the taller block's wall plane; a hip end facing a neighbouring block is clipped like any other buried roof. No overhang crosses a block boundary. Pinned by the roof-line audit on the four combinations (depth diff × story diff).
...
- *rev* **Compatibility is a mapping table, not "defaults".** `normalize` upgrades a version-1 facade:
  ...
  | `pitch_deg` (facade-wide) | both blocks `roof.pitch_deg`; features follow their block |
  | `roof[].kind: hip_end` | kept as a roof feature, unchanged |
  | (absent) | `mirror: false`, `depth: 0`, `stories: 1`, `roof: {gable, x}` per block, `shutters: false`, `story: 1`, `porch.roof: flat` |

  Pin: every saved fixture facade and the canonical render pixel-equivalent before and after the upgrade (exterior view, `--day`). The canonical is today's house expressed in the model: main depth 0, one story, gable/x, batten, no base, body white; garage front, gable/x, batten.
...
- *rev* **Draw honesty**: mergeStatic merges by material, so each distinct (material, body) pair on the house is one more static draw. Budget: ≤ +1 draw per distinct pair, pinned; textures never exceed the mkTex cache's per-house ceiling (recorded in §7).
...
- **What flips, and only these**: the live room camera positions and look-ats; the orbit PIVOT x (-1.8 → +1.8) with stop ANGLES unchanged, so the right chevron, `?angle=`, swipe and ← → still turn the same visual way; `frameZone`'s fixed card-face normals (`house.js` ~11217) and lean-in offsets; card/plaque re-aim; the `garage_front`/driveway constants. Each is a getter over the unflipped constant.
...
- **Pass 1, describe.** The system prompt teaches the block model with one worked example ("a garage on the left showing its gable to the street: garage.roof gable ridge z, orientation front; a hip main with two front gables: main.roof hip ridge x plus two roof features of kind gable"). It asks, in order: `mirror` (garage on the RIGHT as seen from the street → true; on the left or unseen → false); per block stories, roof, cladding, base (material, height, colour), body; street features per block with positions as FRACTIONS of that block's street width (0..1) and spans as fractions; colours by nearest palette name; `unexpressed`. Temperature 0.1, strict JSON. The service snaps fractions to slots and runs `normalize`. The route returns the pass-1 DRAFT and a `critique_token`.
- **Render (in the editor).** The editor loads the draft into its preview at a fixed comparison pose: `--day` lighting (14:00), the street-facing orbit stop nearest the photo's viewpoint (the prompt's pass 1 also reports `viewpoint: left|centre|right`), quality medium. It captures the canvas to PNG (`preserveDrawingBuffer` on for that one frame) and posts it with the token to the critique route.
- **Pass 2, critique.** Photo + the draft render + the draft JSON. The prompt asks for a ranked list of differences in a fixed order — massing and roof forms, materials and base, openings, colours — at most eight, each with a one-line reason, and returns ONE full revised block model (*rev*: not JSON patches; slot arrays reorder under `normalize`, so index paths land on the wrong feature). The service validates the revision through `normalize` on a copy. The result is `{draft, revised, reasons, unexpressed}`: the editor shows both renders, the person picks either; nothing auto-saves; review-before-save as today. If pass 2 fails, times out or returns an invalid model, the pass-1 draft is the result and the notes say so.
...
*rev* Ceilings set BEFORE the first build, not rebaselined after: `buildMs` ≤ 1500 with two stories on both blocks; exterior in-frustum ≤ 1403 + the itemised story/feature delta the plan states per variant (canonical, mirror, two-story, side garage, brick, combined); static draws ≤ today + 1 per distinct (material, body) pair; texture count ≤ the mkTex ceiling recorded at the arc's first commit. Every number lands in §9 with its ceiling beside it.
