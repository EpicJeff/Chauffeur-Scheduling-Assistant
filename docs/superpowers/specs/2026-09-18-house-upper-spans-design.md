...
- Spans on one face are non-overlapping and sorted by slot. Overlap is resolved by `_resolve_exclusive`: first wins, the loser is dropped with a note. Two spans that touch AND carry an identical roof (form, ridge, pitch) are merged into one — there is no seam between them. Two touching spans with different roofs stay two volumes with a seam wall between them (§3).
...
- `slot_table(blocks, upper)` publishes per-slot `eave`: `2·EXT_TOP4` for a slot covered by an upper span, `EXT_TOP4` otherwise. Python's table and house.js's table stay in step (arc 2's rule); a two-story slot's features are placed off this number in both.
...
- **Canonical** is unchanged: `upper: []`. Every canonical pin holds byte-for-byte.
...
- Editor pins (the file that holds the cell-editor scenarios today): paint a span from a cell; a span edit from a covered cell edits the owner; the story control shows only inside a span; a V2 facade opens with its span painted; the Blocks panel has no Stories select.
