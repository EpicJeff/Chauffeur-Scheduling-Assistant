...
**Above-eave closure at a seam (one owner, one rule for every roof pairing).** At every seam and at x −7.15 the volume that owns the seam wall (§3 Upper walls: the taller volume; the west volume when both are uppers; main on a tie across x −7.15) also owns the closure above its eave on that plane — its gable infill (ridge x) or hip end (hip) — and that closure is CLIPPED against the neighbour volume's bounded box and deck planes exactly as roofs are. The non-owner builds no closure on that plane. Consequences, each a pin: wing|upper — the upper owns, its wall runs to its eave, and the wing's infill is not built (it would be entirely inside the upper's box); upper|upper with different roofs at equal eave (e.g. a ridge-x span beside a ridge-z span) — the west span owns, and its gable triangle above the neighbour's sloping deck is built and clipped to what stands proud of that deck, so no triangular hole opens above the neighbour's roof; identical roofs at equal eave — merged in normalize, no seam.
...
Live:
- `test_house_shell_live` — **partial upper on the main block** (span slots 9–12, gable ridge z; wings ridge x): `main_u1_*` walls and roof registered, `main_w0_*`/`main_w2_*` roofs registered, exactly one wall per seam (exact registry set), roof-line audit clean at both seams and at x −7.15, every room view pixel-unobstructed from its camera (kitchen and living look down through the upper), partitions under the wings vault and under the upper cap. **Two spans on the garage block** (bay slots 0–2 ridge z, mudroom slots 3–5 ridge x): two volumes at equal eave, one seam owned by the bay span (west), the bay's closure on the seam clipped against the mudroom span's deck and non-empty (no hole above the mudroom roof: a pixel check from orbit stop 1 finds no sky through the seam), the mudroom span builds no closure on that plane, the shared plane x −7.15 closed once against a one-story main, the garage door and `garage_front` marker unchanged. The existing `scenario_two_stories_are_cut_like_fabric` is re-expressed as a full-face span on both blocks with the same assertions.
- `test_house_facade_live` — a full-face span on both blocks reproduces arc 2's two-story pins (roof piece names, mesh count): the V2-equivalence pin.
- `test_house_variants_live` — `two_story` becomes the full-face span (delta unchanged, +22/+44); a new `partial_upper` variant (main span 4 slots ridge z + garage span 3 slots ridge z) with its own PAIRED delta, measured first on a bare server and then ceilinged at measured + headroom; `combined` gains a partial span on each block and its delta is re-measured. `buildMs` law unchanged: `max(1500, base + 250)`.
...
The canonical build is byte-identical, so B0 is unchanged and no ceiling is rebaselined. Every variant is paired against a base measured in the same run (arc 2's rule). The `partial_upper` delta is a NEW number: measured before it is ceilinged, recorded in §9 with the command line. Exterior in-frustum meshes, per-view draw calls (measured, never inferred) and texture count as arc 2 §7.
...
Filled at wrap: the farmhouse render before/after with the partial upper; the `partial_upper` and re-measured `combined` deltas with B0 beside them; the exact registry sets for the two span scenarios; every deviation with its ruling; the parked list.
