...
The draw-budget tool (`tools/house_probe.py --budget`) reports build time, unique materials and geometries. Its `buildMs` reading was silently broken for the whole arc: the injected `render()` wrapper latched on the first call it saw, which at PBR (`quality=high`) tiers is `PMREMGenerator`'s own internal render of a near-empty scene, not the house scene — so every high-tier `buildMs` quoted in Tasks 1-8 (a constant 6-8ms) was dead, while `quality=low` readings (no PMREM) were real throughout. Fixed in the closing fix wave (the latch now requires the rendered scene to be the real one, `children.length > 8`); corrected exterior readings are `quality=high` ~536-601ms and `quality=low` ~108ms, both dominated by renderer/context setup and, at high tier, PMREM environment-map generation and shader compilation rather than by geometry construction — consistent with Task 4's independent finding that the geometry cache's own contribution to build time is noise-level at this scene's scale. L7 holds on the evidence that does bear on it: nothing in the batching work adds a render call or a geometry rebuild to either tier's setup cost. Scene batches via materials/geometries cached, garden instanced, fabric merged.
