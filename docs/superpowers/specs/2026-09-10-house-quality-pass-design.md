...
**Per-room in-frustum mesh budgets, final.** High tier, three baselines per room:

| Room | (a) Batching floor (arc start) | (b) Room-pass result vs its ceiling (post-kit base) | (c) Arc-final closing gate |
|---|---|---|---|
| Exterior | 1134 | 1231 vs 1413 (base 1229) | 1231 |
| Kitchen | 363 | 415 vs 419.75 (base 365) | 418 |
| Living | 682 | 694 vs 784 (base 682) | 730 |
| Mudroom | 373 | 376 vs 430 (base 374) | 382 |
| Garage | 526 | 539 vs 606 (base 527) | 539 |

Exterior progression reflects Task 9's telemetry-leak fix (1134→1212) and Task 10's curb-scene completion (1212→1229→1231). The (b)-vs-(c) deltas for kitchen, living, and mudroom come from later rooms' content (cars, exterior) visible cross-room through the open floor plan; exterior and garage are unchanged because nothing built after their own room pass is visible from those cameras. Kitchen's arc-final margin against its own room ceiling is now ~1.75 (418 vs 419.75) — tight, worth naming. Every room finished under its own ceiling; per-room slack is in the table above.

**Build time.** Pre-AO (K1–K3): 511–558ms; post-AO (K4): 1108ms high, 618ms medium; final after lighting (Task 11): 1103–1280ms high, staying under the 1500ms rule. The arc-final closing-gate probe recorded 1359ms — outside that 1103–1280ms sample range, consistent with the documented run-to-run probe noise, still comfortably under the 1500ms ceiling. DIRS 6, STEPS 4.

**Lighting.** Sun rake: SUN_OFF (9, 24, 24); fill north-wall static at wallB centre (0, 2.8, −5.55); kitchen gate 1.0175x; gradient maps on shared floor/wall/counter textures via applyScenery exemption. Hood tier fix: the colour half is real; the authored emissive half can never render (`applyState`'s zone sweep overwrites it in both lit and calm states, pre-existing — see Accepted miss 5). Baked gradients closed the low/medium tier read.

**Rooms.** Kit slices K1–K5 landed v2.479.0–v2.483.1 (chamfer, lathe/sweep profiles, physical materials, AO bake, contact discs); room passes R1–R5 completed v2.484.0–v2.488.0 (living detail, kitchen showcase, mudroom+pantry, garage+cars, exterior; garage-leak follow-up v2.487.2 landing between R4 and R5); lighting slice L shipped v2.489.1; wrap docs landed v2.489.2–v2.489.3. A user-reported lean-in/out texture leak (`mkTex` overwriting a cached `CanvasTexture` without disposing the one it replaced) was root-caused and fixed at v2.490.0–v2.490.2 (dispose-on-overwrite at the source, plus the fridge-magnets and carPlates siblings brought into the same discipline, pinned by three runtime scenarios). This final-fix wave — the whole-branch review's three fixes plus fold-in minors — lands v2.490.3. Screenshots live at `%LOCALAPPDATA%/Temp/house_quality/` in subdirectories K1–K5 (kit slices), R1–R5 (room probes), L (pre-rake), L/fix1 (lighting fix round), final-fix (this wave's pixel proof).

**Retired from the open-items punch list.** Built-ins read as casework: face frames, shelf lips, hardware landed (R1). Cars block eliminated: seven parametric body shapes with separate lathe and lights per body type, all in-frustum (R4). Armchair read fixed by composition move and prop rules (R1). Minivan vs van separation by roof height and bonnet length (R4). Bus-arm STOP lettering now reads at curb camera (R5). Kitchen counter tier consistency: medium/high `kWoodK` mismatch resolved (R2). Wall gradients baked into shared canvas textures (L). Sun rake moved SUN_OFF off the frontal axis (L).

**Accepted misses.** The mkTex/heroTex texture leak this list carried as accepted through v2.489.1 is RESOLVED, not accepted: v2.490.0–v2.490.2 fixed it at the source (dispose-on-overwrite) plus its fridge-magnets and carPlates siblings, pinned by three runtime scenarios (see the Rooms note above) — it drops off this numbered list. (1) Fence gate-hardware anchored on shared corner post reads as hardware-on-a-post, not a literal gate — legibility noted; fence() has no gap logic and composition freeze holds. (2) house_probe.py infra: cannot force bus_active + curb cam natively (one-off monkeypatch used for probes); --day flag promoted to allow PIL-gated lighting runs after sunset. (3) Intermittent /api/v2/chat/stream traceback on two probe runs — unrelated to house, pre-existing backend issue. (4) Cross-session probe drift: material counts ±~6, inFrustum ±~6 on 4/5 views — environmental, not code; A/B method absorbs it equally. (5) Hood tier fix's authored emissive half never renders: `applyState`'s zone sweep overwrites the hood-underside emissive in both lit and calm states, pre-existing — only the colour half of the fix is real. (6) K4's clone-budget fallback is ENGAGED, not just plan-authorized: 0 clones were taken this arc, so multi-worn props stayed unbaked for AO — the shell/occlusion arc should read this as the true AO coverage on entry, not a temporary shortfall.
