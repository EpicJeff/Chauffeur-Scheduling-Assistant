...
- [ ] **Step 1: Build the street face (spec section 6 REVISED — an elevation, not a slab).** Derive the great-room south edge + width from the existing floor plate extents (`Box3` on the great-room floor group at build). Compose: OFFSET FRONT DOOR (decorative-door casing/panel idiom ~:2102, no new zone) under a COVERED GABLED PORCH — two posts, small gable in the garage gable's pitch family, stoop + one step, second coach-lamp instance beside the door; living-side window pair + kitchen-side window with aligned heads (pane idiom; panes are part of the piece and hide with it — transparent-never-MERGES only constrains mergeStatic, visibility grouping is unaffected); exterior face = shared siding-mapped material (cache hit — R5 normal map rides along); interior face = plaster with `wallGradTex` (T11 idiom, mapped ⇒ scenery-exempt); trim + kick per R5 hardware scale. Everything (door, porch, windows, trim) lives in `southWallG` — one fabric piece. Stamp `room: 'kitchen'`. `regFabric(southWallG, { name: 'south_wall', n: [0, 0, 1], box: swBox })`.
- [ ] **Step 2: Complete the roofline as ONE family.** Read the architect-pass main roof (both slopes + fascia): close any cutaway/gap over the great-room front; exterior walls RISE TO THE ROOF — build gable-end infill panels closing the 5.6→~7.0 wedge (first-attempt finding; its reference diff: scratchpad t4-killed-attempt.diff — reference only, re-derive against the live file); porch gable + garage gable share the pitch family; re-check mudroom/living roof boxes against the compass shots and adjust pitch/eave into the family if they float (composition change authorized in THIS task only). Watertight from EXT_POS and a 4-shot compass orbit. Register `roof_south` (n = the slope's true outward normal computed from its geometry at build, not hand-typed), `roof_north`, `north_wall`, `east_wall`, `garage_shell` (walls+gable one piece; `garage_door` stays its own piece).
...
- [x] **Step 1:** RED first: the real-mouse scenario (exterior tap on mudroom-roof region -> mode mudroom; kitchen radio tap -> mode living; kitchen fridge/board tap -> lean; island tap -> mode stays kitchen; sky tap -> exterior). Screen points derived from projected piece/zone boxes at runtime (evaluate a projector using the exported hooks), never hard-coded pixels.
- [x] **Step 2:** regFabric room field; registrations updated (south_wall/roofs-over-great-room -> kitchen, mudroom_roof -> mudroom, living_roof + east_wall -> living, garage_shell + garage_door -> garage, west_wall/west_skirt by adjacency); stamp at registration.
- [x] **Step 3:** onTap rewrite per section 5 rules 1-5; delete the cross-room null; own-room zoneless props inert; sky/yard exits; kitchen two-step stays.
- [x] **Step 4:** Verify: scenario GREEN; verdict table, room bounds, geometry, and mesh budgets match an identical-fixture control run. Record buildMs separately against the 1500 ms target; elapsed time is not bit-identical between runs. Leak scenarios green; full sweep. Commit v2.496.0 `'The house learns to be walked (v2.496.0)'`, push.
