...
### Touch-first interaction discovery (v2.498.0)

The persistent cutaway edge network obscured the rooms and conveyed no useful action on a wall-mounted touch display. Ghost verdicts now hide the obstructing fill cleanly without drawing their wireframe siblings. A single warm pulse tours reachable interaction targets: registered room entrances outside and registered zone groups inside. It carries a short label, rests between targets, never intercepts pointer input, hides during focused detail, and respects reduced-motion settings. Window illumination remains an atmospheric lived-in cue rather than an undocumented interaction state.

Verification: `scratch/interaction-hint-exterior.png` and `scratch/interaction-hint-kitchen.png` show the pulse in both navigation layers and a clean interior cutaway with **0 visible ghost edges**. The focused live-house regression passed, the full suite passed **222/222 files in 355 seconds**, and `node --check static/house.js` plus `git diff --check` pass.

### Persistent feature markers (v2.498.1)

Discovery no longer makes the user wait for a rotating target. Every reachable entrance or room object keeps a labeled ring on screen. The exterior mapping follows the actual plan: the porch/front door opens the front Living room, and the side patio slider opens the rear Kitchen; the front wall and roof likewise belong to Living while the rear roof and long side elevation belong to Kitchen. Exterior room rings contain the feature icons available inside—Living room previews Music and Critters, while Kitchen previews Moments, Meals, Lists, Calendar, and Weather. Interior rings carry the individual object's icon. The rings retain a gentle staggered wave, remain pointer-transparent, disappear during focused detail, and become static when reduced motion is requested.

Verification: `scratch/interaction-hint-exterior.png` shows all four labeled room markers together with Living room on the porch door and Kitchen on the rear patio side; `scratch/interaction-hint-kitchen.png` shows all five individually iconed kitchen objects. Real-pointer regressions cover the front door, front facade, side elevation, and rear patio slider. The full suite passed **222/222 files in 338 seconds**; `node --check static/house.js` and `git diff --check` pass.

### Truthful curb bus (v2.498.2)

The curb builder previously called `bus_active()` and stored the answer in a variable named `near`. That predicate intentionally accepts an active sensor or a recently updated tracker without considering distance, day, or school calendar, so a Sunday-night tracker update could place the model at the house. The house-specific gate now requires a child, `school_in_session(day)`, `bus_active(member)`, and `bus_is_near(member)`. The broader bus map and notification behavior is unchanged.

Verification: focused house-state coverage proves an active nearby bus appears on a school day, an active distant bus stays absent, and Sunday at 9:30 PM stays absent. The complete bus-service test passes unchanged, and the full suite passed **222/222 files in 336 seconds**; `git diff --check` passes.
