        ...
        if frame.f_code.co_name == 'find_slots':
            ...
            if 'slot_local' in loc and loc['slot_local'].hour == 12 and loc['slot_local'].minute == 0:
                if frame.f_lineno == 513:
                    print(f"Overlap with {getattr(loc.get('e'), 'name', 'unknown')} at {loc.get('e').start}")
                    print(f"travel_mins: {loc.get('travel_mins')}, dynamic_buffer: {loc.get('dynamic_buffer')}")
                    print(f"slot_start: {loc.get('slot_start')}, slot_end: {loc.get('slot_end')}")
                    print(f"e.start: {loc.get('e').start}, e.end: {loc.get('e').end}")
    return trace_lines
