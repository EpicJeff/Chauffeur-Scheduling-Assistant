...
def scenario_side_door_settings_round_trip():
    ...
    raw['blocks']['garage']['side_door'].update(third_bay=True, front_setback=0.75, projection=1.8)
    spec, _ = hf.normalize(raw)
    ...
    raw['blocks']['garage']['side_door']['projection'] = 99
    check(hf.validate_block_model(raw), 'oversized projection rejected')
    check(hf.normalize(raw)[0]['blocks']['garage']['side_door']['projection'] == 3, 'projection clamped')
