"""The Home in real chromium against the served app: boot, the two-level
camera entering and leaving, and the texture/geometry leak-and-rebuild
regression guards.
...
Task 7 split this ~21-scenario file four ways so tools/test.py's parallel
sweep can spread it across workers instead of serializing behind one
8-minute file. This file keeps the lifecycle/boot/leak scenarios; the
shell/mask/vault/clipper/convexity/study-box scenarios live in
test_house_shell_live.py, navigation/orbit/swipe/idle in
test_house_nav_live.py, and the facade pin/worst case/roof-line audit/
saved-facade reproduction in test_house_facade_live.py. Shared helpers
(check, _seed, INVARIANT_JS, DAY_LOCK_JS, the vault pitch) live in
house_live_common.py — imported, never run on its own.
...
from house_live_common import check, _seed, INVARIANT_JS, DAY_LOCK_JS
