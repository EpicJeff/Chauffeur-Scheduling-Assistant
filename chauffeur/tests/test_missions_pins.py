...
def scenario_mission_tier_never_serves_free_models():
    ...
    models = list(model_pools.models_for('mission', {}))
    check(models, "mission tier resolves to at least one model")
    for m in models:
...
def scenario_end_to_end_runthrough_executes_nothing():
    ...
    orig_settings, orig_llm = storage.get_settings, missions._llm
    try:
        ...
        storage.get_settings, missions._llm = orig_settings, orig_llm
