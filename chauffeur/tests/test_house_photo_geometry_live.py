...
def main():
    ...
    data=json.loads((Path(__file__).parent/'fixtures/house_photo_missing_openings.json').read_text())
    spec,_=hf.normalize(data['before_normalization'])
    restore_missing_upper_windows(spec,data['observations'])
    ...
    try:
        with served.browser() as page:
            ...
            for mode in ('replay','unmirrored','no-upper'):
                ...
                assert 'garage_block_roof_attic_window' in rows, rows
                assert not any(n.startswith('facade_garage_block_gable_0') for n in rows)
                ...
                if os.environ.get('HOUSE_SHOTS'):
                    ...
                    page.screenshot(path=str(out/('photo-geometry-'+mode+'.png')))
    finally:served.stop()
    print('PASS: replayed windows, single end gable, attic window; mirrored/unmirrored/no-upper')
