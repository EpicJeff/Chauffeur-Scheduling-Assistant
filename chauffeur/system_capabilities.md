...
- **Errand Rules Engine**: Independent from global Routing Rules, the Errand Rules Engine matches created errands (via keywords and locations) and uses a `constraint_type` paradigm (Driver Assignment, Passenger Constraints, Time of Day, Buffer & Tolerance, Grouping) to automatically enforce rules. It uses an allowed/required/prohibited 3-state logic for drivers and passengers.
