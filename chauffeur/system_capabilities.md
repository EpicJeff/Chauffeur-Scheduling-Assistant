...
- **Timezone and Remote Event Scheduling**: When scheduling POIs in a Draft Trip that lacks a specific location or timezone, the engine infers the timezone from the POIs to avoid scheduling offsets. Furthermore, when computing dynamic travel buffers against the user's personal calendar events, any travel time exceeding 180 minutes is truncated to 0 under the assumption that the event is either remote or skipped, preventing multi-state travel times from blocking out entire vacation days.
- **Ideal Time Constraints**: For POIs that specify an `ideal_time_start` and `ideal_time_end` window (like dining reservations), the solver evaluates whether the **start time** of the POI falls within the window (plus a 15-minute grace period). It intentionally does not constrain the end time, allowing long-duration activities (like a 90-minute dinner) to start inside a narrow window without failing mathematically.
...
- **Smart Scheduling Insights**: The solver actively detects cross-day geographical inefficiencies. If two POIs are very close (<= 15 mins travel) but are scheduled on different days, the solver yields `insight` payloads via NDJSON stream to trigger a UI recommendation modal.
