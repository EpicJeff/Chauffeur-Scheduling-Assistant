...
- **Trip POIs as Bounded Errands**: Trip Points of Interest (POIs) act as specialized dynamic errands. They are automatically constrained strictly to the date range of the Trip event. The solver schedules them opportunistically within the trip's time window, effectively building an auto-itinerary without requiring explicit date configuration from the user.
