...
- **LLM-output repair guards** (`services/trip_planner.py`): applied server-side before persisting a generated plan, zero extra requests. (1) `_repair_accommodation_overlaps` — overlapping accommodation **date ranges** are structurally invalid (they poison the scheduler's day→home-base map): stays overlapping an already-saved stay are dropped, zero-night stays are dropped, an "umbrella" stay spanning ≥2 other stays (whole-trip hotel emitted alongside per-leg ones) is dropped, and remaining overlaps are clipped forward (the later check-in wins). Repeating a *location* is always fine (Paris legs 1 and 4 both keep their hotel); only date overlap is repaired. Undated entries pass through. The dedup against existing stays is date-aware: a re-suggestion of an already-saved hotel name is kept when it carries a new non-overlapping date range (a return stay) and dropped only when undated or date-overlapping. (2) `_anchor_fields` — a single-day `is_background` suggestion lasting under `ANCHOR_MIN_DURATION_MINS` (180) is demoted to a regular POI (a 60-min museum visit must not claim a whole day and hijack its distance base); anchors with `days_claimed > 1` or no stated duration keep their flag.
...
- **Multi-leg trips (accommodations as legs)**: each trip day's routing "home base" is the accommodation covering that date (check_in ≤ date < check_out), so POIs gravitate to the leg they're closest to via the distance shaping term — no rules needed. Anchors (background POIs) get the same pull: each candidate claim day is charged −300/haversine-min from that day's accommodation to the anchor, so an anchor claims days inside its own leg (a Riviera anchor never claims Paris-hotel nights). On flexible-date drafts the UI shows accommodation stays as trip-day numbers only ("Day 2", numeric day inputs in the edit dialog) — the mock calendar dates behind them must never be shown to users; real dates appear only once the trip has real dates. The agent expresses legs through `add_trip_accommodation` / `edit_trip_accommodation`, which accept absolute `check_in_date`/`check_out_date` or 1-indexed trip-night ordinals (`check_in_night`, `nights`; night 1 = first night — preferred for drafts since their mock dates are an implementation detail). Legs must be contiguous and non-overlapping (leg N+1 checks in when leg N checks out); omitting dates spans the whole trip. The plan-generation prompts (`generate_trip_plan`, `generate_trip_accommodations`) are instructed to emit one accommodation per described leg in the user's stated order with contiguous dates. For hard leg fences, add a `day_restriction` TripRule with `trip_days`.
