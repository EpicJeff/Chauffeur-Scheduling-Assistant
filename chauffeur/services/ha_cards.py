    ...
    for s in ha_api.get_states(ttl=10) or []:
        ...
        if eid in wanted:
            out[eid] = {
                ...
                # Cards compare these by identity to decide whether to
                # re-render. Ours are rebuilt each poll, so the comparison
                # always says "changed" — correct, if slightly eager.
