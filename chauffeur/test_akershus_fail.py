...
with open("services/trip_planner_hacked9.py", "w") as f:
    f.write(code)

sys.path.insert(0, ".")
import services.trip_planner_hacked9
services.trip_planner_hacked9.maps = services.trip_planner.maps
services.trip_planner_hacked9.calendar = sys.modules['services.calendar']

for p in trip.pois:
    if "Akershus" in p.name:
        print("Testing Akershus forced failure...")
        p.valid_days_of_week = [0]
        res = services.trip_planner_hacked9.schedule_poi(trip, p, bounds=epcot_bounds)
        print("RESULT:", res)
